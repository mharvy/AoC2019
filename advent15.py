...
class Droid(object):
	def __init__(self, cpu, pos):
		...
		self.pos = pos
		self.start = pos
		# This helps with part 2
		self.target = (-1, -1)
		# For keeping track
		self.layout = {}
		# For visual
		self.grid = [['0' for i in range(80)] for i in range(50)]

	# Part 1
	def find_target(self):
		self.layout[self.pos] = '.'
		self.grid[self.pos[1]][self.pos[0]] = 'S'

		prev = (0, 0)
		while 1:
			# First check if up is undiscovered
			up = self.layout.get( (self.pos[0], self.pos[1]+1), 'n')
			down = self.layout.get( (self.pos[0], self.pos[1]-1), 'n')
			right = self.layout.get( (self.pos[0]+1, self.pos[1]), 'n')
			left = self.layout.get( (self.pos[0]-1, self.pos[1]), 'n')

			# First check undiscovered
			if left == 'n':
				response = self.cpu(3)
				if response[0] == 2:
					self.pos = (self.pos[0]-1, self.pos[1])
					self.target = self.pos
					self.grid[self.pos[1]][self.pos[0]] = 'T'
					for r in self.grid:
						print(' '.join(r))
					return self.pos
				if response[0]:
					...
					self.pos = (self.pos[0]-1, self.pos[1])
					self.layout[self.pos] = '.'
					self.grid[self.pos[1]][self.pos[0]] = 'C'
					continue
				else:
					self.layout[(self.pos[0]-1, self.pos[1])] = '#'
			...
			if down == 'n':
				...
				if response[0]:
					...
					self.grid[self.pos[1]-1][self.pos[0]] = '#'
					down = '#'
			if right == 'n':
				response = self.cpu(4)
				if response[0] == 2:
			...
			# Next, check discovered with preference on not visiting last node
			if left == '.' and prev != (self.pos[0]-1, self.pos[1]):
				response = self.cpu(3)
				if response[0]:
					prev = self.pos
					self.grid[self.pos[1]][self.pos[0]] = '.'
					...
					self.grid[self.pos[1]][self.pos[0]] = 'C'
					continue
			if down == '.' and prev != (self.pos[0], self.pos[1]-1):
				response = self.cpu(2)
				if response[0]:
					prev = self.pos
					self.grid[self.pos[1]][self.pos[0]] = '.'
					...
					self.grid[self.pos[1]][self.pos[0]] = 'C'
					continue
			if right == '.' and prev != (self.pos[0]+1, self.pos[1]):
				response = self.cpu(4)
				if response[0]:
					prev = self.pos
					self.grid[self.pos[1]][self.pos[0]] = '.'
					...
					self.grid[self.pos[1]][self.pos[0]] = 'C'
					continue
			if up == '.' and prev != (self.pos[0], self.pos[1]+1):
				response = self.cpu(1)
				if response[0]:
					prev = self.pos
					self.grid[self.pos[1]][self.pos[0]] = '.'
	...
	def find_shortest_path(self):
		# Use bfs through maze to find shortest path
		q = [(0, self.start)]
		visited = set()

		while len(q) != 0:
			cur_len, cur_pos = q.pop(0)
			visited.add(cur_pos)

			if self.grid[cur_pos[1]][cur_pos[0]] == 'T':
				return cur_len

			# Add neighbors
			up = (cur_pos[0], cur_pos[1] + 1)
			down = (cur_pos[0], cur_pos[1] - 1)
			right = (cur_pos[0] + 1, cur_pos[1])
			left = (cur_pos[0] - 1, cur_pos[1])
			if self.grid[up[1]][up[0]] not in ['#', '0'] and up not in visited:
				q.append((cur_len + 1, up))
			if self.grid[down[1]][down[0]] not in ['#', '0'] and down not in visited:
				q.append((cur_len + 1, down))
			if self.grid[right[1]][right[0]] not in ['#', '0'] and right not in visited:
				q.append((cur_len + 1, right))
			if self.grid[left[1]][left[0]] not in ['#', '0'] and left not in visited:
				q.append((cur_len + 1, left))
...
def main():
	...
	droid = Droid(cpu, (25, 25))
	droid.find_target()
	print(droid.find_shortest_path())
	print(droid.fill_region())
