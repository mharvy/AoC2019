...
class PaintBot(object):
	def __init__(self, brain, direction, position):
		self.brain = brain
	...
	def paint(self, layout):
		...
		halt = False
		while not halt:
			to_color, halt = self.brain(on_color)
			if halt:
				break
			layout[tuple(self.position)] = to_color

			to_turn, halt = self.brain(on_color)
			if halt:
				break
			if to_turn:  # right
...
def main():
	...
	for y in range(y_min, y_max + 1):
		...
		for x in range(x_min, x_max):
			print((x, -1 * y))
			drawn[-1] += ('#' if (x, y) in layout and layout[(x, y)] else '.')
